...
function testLib(libName) {
  ...
  try {
    execSync(
      `ng test ${libName} --watch=false --browsers=ChromeHeadless --code-coverage=true`,
      opts
    );
  } catch (err) {
...
function testAll() {
  ...
  try {
    execSync(
      `node ./node_modules/istanbul/lib/cli.js report --dir ./coverage/ html`,
      opts
    );
    execSync(
      `node ./node_modules/istanbul/lib/cli.js report --dir ./coverage/ json`,
      opts
    );
  } catch (err) {
    console.log("ec", exitCode);
